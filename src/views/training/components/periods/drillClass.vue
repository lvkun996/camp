<template>
    ...
    <el-collapse accordion>
        <el-collapse-item>
            ...
            <el-tag @click="confirmAdd">确定添加</el-tag> 
            <div>
                 <el-table
...
export default {
    ...
    methods: {
        // 日期格式转换
        onSaveTime () {
            this.studyTask.startTime = this.$moment(this.time[0]).format('YYYY-MM-DD HH:mm:ss')
            this.studyTask.endTime = this.$moment(this.time[1]).format('YYYY-MM-DD HH:mm:ss')
        },
        // 添加流程和打卡
        async confirmAdd () {
